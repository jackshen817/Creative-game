...
#提取上海每日温度、湿度、气压文件
dfall_chongqing_day_temperature = pd.read_csv(r'E:\pycharm\Creative game\Average_data\operation_result\chongqing\chongqing_average_everyday_temperature.csv',parse_dates=['date'],infer_datetime_format=True)
dfall_chongqing_day_humidity = pd.read_csv(r'E:\pycharm\Creative game\Average_data\operation_result\chongqing\chongqing_average_everyday_humidity.csv',parse_dates=['date'],infer_datetime_format=True)
dfall_chongqing_day_pressure = pd.read_csv(r'E:\pycharm\Creative game\Average_data\operation_result\chongqing\chongqing_average_everyday_pressure.csv',parse_dates=['date'],infer_datetime_format=True)

#处理上海每日温度数据
dfall_chongqing_day_temperature.replace(999999,np.nan,inplace=True)
dfall_chongqing_day_temperature.fillna(method='ffill',inplace=True)

#处理上海每日湿度数据
dfall_chongqing_day_humidity.replace(999999,np.nan,inplace=True)
dfall_chongqing_day_humidity.fillna(method='ffill',inplace=True)

#处理上海每日气压数据
dfall_chongqing_day_pressure.replace(999999,np.nan,inplace=True)
dfall_chongqing_day_pressure.fillna(method='ffill',inplace=True)


dfall_temperature_1 = dfall_chongqing_day_temperature.rename(columns={'date':'ds', 'temperature':'y'})
...
dfall_humidity_1 = dfall_chongqing_day_humidity.rename(columns={'date':'ds', 'humidity':'y'})
#dfall['y'] = np.log(dfall['y'])
...
dfall_pressure_1 = dfall_chongqing_day_pressure.rename(columns={'date':'ds', 'pressure':'y'})
#dfall['y'] = np.log(dfall['y'])
...
df1['yhat'] = (df1['yhat'] * (dfall_chongqing_day_temperature['temperature'].max() - dfall_chongqing_day_temperature['temperature'].min())) + dfall_chongqing_day_temperature['temperature'].min()

df2['yhat'] = (df2['yhat'] * (dfall_chongqing_day_humidity['humidity'].max() - dfall_chongqing_day_humidity['humidity'].min())) + dfall_chongqing_day_humidity['humidity'].min()

df3['yhat'] = (df3['yhat'] * (dfall_chongqing_day_pressure['pressure'].max() - dfall_chongqing_day_pressure['pressure'].min())) + dfall_chongqing_day_pressure['pressure'].min()
...
#data1.to_csv('data_chongqing_forecasting_temperature_data.csv')
...
#data2.to_csv('data_chongqing_forecasting_humidity_data.csv')
...
#data3.to_csv('data_chongqing_forecasting_pressure_data.csv')

forecast_chongqing_sum_data = pd.concat([data1, data2, data3], axis= 1)
forecast_chongqing_sum_data.to_csv("forecast_chongqing_sum_data.csv")
